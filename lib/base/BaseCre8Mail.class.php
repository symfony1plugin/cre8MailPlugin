...
class BaseCre8Mail 
{
  ...
  static public function getBodyAndAlternateFromTemplate($template, $templateParameters = null)
  {
    // Check if template exists in database
    $cre8EmailTemplate = Cre8MailTemplatePeer::getBySlug($template);
    if (!$cre8EmailTemplate) {
      throw new Exception('Email Template does Not exist in DB');
    }
    
    $body = self::getTemplateContent($cre8EmailTemplate->getBody(), $templateParameters);
    $part = self::getTemplateContent($cre8EmailTemplate->get, $templateParameters);
  	return array($body, @$part['content'] ? $part : null);
  }
  
  static public function getTemplateContent(Cre8MailTemplate $cre8MailTemplate, $parameters)
  {
    $vars = array();
    $cre8MailVariables = array();
    foreach($cre8MailTemplate->getCre8MailTemplateVariablesJoinCre8MailVariable() as $cre8MailTemplateVariable) {
      $cre8MailVariables[] = $cre8MailTemplateVariable->getCre8MailVariable();
    }
    
    foreach ($cre8MailVariables as $cre8MailVariable)
    {
      if(array_key_exists($cre8MailVariable->getName(), $parameters)) {
        $vars[$cre8MailVariable->getName()] = $parameters[$cre8MailVariable->getName()];
      } elseif ($cre8MailVariable->getIsComplex()) {
        $cre8ComplexTemplateVariable = new Cre8ComplexTemplateVariable();
        if(method_exists($cre8ComplexTemplateVariable, 'get' . ucfirst($cre8MailVariable->getName()))) {
          $vars[$cre8MailVariable->getName()] = call_user_func(array('Cre8ComplexTemplateVariable', 'get' . ucfirst($cre8MailVariable->getName())));
        }
      }
    }
    ...
    {
      $cre8MailTemplate->setBody(preg_replace('/\(\+' . $key . '\+\)/i', $val), $cre8MailTemplate->getBody());
      $cre8MailTemplate->setTextBody(preg_replace('/\(\+' . $key . '\+\)/i', $val), $cre8MailTemplate->getTextBody());
    }
  ...
  {
    return preg_replace_callback('%(src|href)=["|\'](/.*)("|\')%i', array('cre8Mail', 'compute_replacement'), $txt);
  }
  ...
  protected static function getContent($content, array $embedded_images = array())
  {
    ...
    return self::replaceEmbeddedImages($string, $embedded_images);
  }
  ...
  protected static function replaceEmbeddedImages($string, array $embedded_images = array())
  {
    ...
    /*
    echo '<pre>';
    print_r(array('transform_key' => $transform_key, 'keys' => $keys, 'values' => $values, 'txt' => $string, 'retVal' => str_replace($keys, $values, $string)));
    echo '</pre>';
    die('--- TEST ----');
    */
  ...
  {
    sfApplicationConfiguration::getActive()->loadHelpers(array($helper));
  }
}